...
select tran_date,shift,eb_no,mechine_name,quality,vps.attendance_type, sum(prod) prod,sum(atthrs) atthrs,
        ...
        case when shift<>'C' then sum(atthrs)/8 else sum(atthrs)/7.5 end noofwinders  from EMPMILL12.view_proc_spellwindingdata vps where tran_date between '{start_date}'
		and '{selected_date}'	
