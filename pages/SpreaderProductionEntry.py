...
with tab1:
    ...
    try:
        ...
        if not day_rows.empty:
            ...
            # Build display WITHOUT showing internal group; add an autogenerated index column.
            # We'll keep the internal row id hidden but available for deletion actions.
            display_cols = [
                'spreader_prod_entry_id','bin_no','entry_time','spell','Spreader Display','Jute Quality','no_of_rolls','wt_per_roll','Produced Weight (kg)','Produced Weight (MT)','trolley_no'
            ]
            day_display = day_rows[display_cols].rename(columns={
                'spreader_prod_entry_id':'_row_id',
                'bin_no':'Bin',
                'entry_time':'Hour',
                'spell':'Spell',
            ...
            # Deletion state (legacy keys cleanup not required now)
            if '_prod_delete_pw' not in st.session_state:
                st.session_state['_prod_delete_pw'] = ''

            # Add delete icon column (will populate after building totals row)
            # Totals row
            totals = {
                '_row_id':'',
                'Bin': 'Total',
            ...
            PASSWORD = "9454368"
            # Prepare Delete + ID view
            day_display['Delete'] = False
            # Rename id for display
            day_display['ID'] = day_display['_row_id']
            # Mark totals row delete blank
            if len(day_display) and str(day_display.iloc[-1]['Bin']).lower() == 'total':
                day_display.at[day_display.index[-1], 'Delete'] = ''
                day_display.at[day_display.index[-1], 'ID'] = ''
            display_order = ['Delete','ID','Bin','Hour','Spell','Spreader','Jute Quality','Rolls','Wt/Roll (kg)','Produced Weight (kg)','Produced Weight (MT)','Trolley']
            work_df = day_display[display_order].copy()
            config = {
                'Delete': st.column_config.CheckboxColumn("🗑️", help="Select rows then click Delete Selected", default=False),
                'ID': st.column_config.TextColumn("ID", help="spreader_prod_entry_id", disabled=True)
            }
            # Initialize session_state helpers for new callback-based deletion
            if '_prod_pw_input' not in st.session_state:
                st.session_state['_prod_pw_input'] = ''
            if '_prod_delete_trigger' not in st.session_state:
                st.session_state['_prod_delete_trigger'] = False
            if '_prod_last_deleted' not in st.session_state:
                st.session_state['_prod_last_deleted'] = []

            # Build a working DF with a checkbox column x (mimic provided pattern)
            pwd_label_col = 'Delete'
            work_df = work_df.rename(columns={'Delete': 'x'})
            # Protect total row: keep boolean False (avoid mixing types which breaks editing)
            if len(work_df) and str(work_df.iloc[-1]['Bin']).lower() == 'total':
                work_df.at[work_df.index[-1], 'x'] = False
            # Ensure checkbox column is boolean dtype
            try:
                work_df['x'] = work_df['x'].astype('boolean')
            except Exception:
                pass

            def prod_delete_callback():
                # Determine edited rows flagged for deletion
                edited_rows = st.session_state['prod_editor']['edited_rows'] if 'prod_editor' in st.session_state else {}
                to_delete_ids = []
                for ridx, edits in edited_rows.items():
                    # ridx corresponds to dataframe positional index
                    try:
                        if edits.get('x') is True:
                            row_id_val = st.session_state['_prod_source_df'].iloc[ridx]['ID']
                            bin_val = str(st.session_state['_prod_source_df'].iloc[ridx]['Bin']).lower()
                            if bin_val != 'total' and row_id_val not in ('', None):
                                to_delete_ids.append(row_id_val)
                    except Exception:
                        continue
                st.session_state['_prod_pending_delete_ids'] = to_delete_ids
                st.session_state['_prod_delete_trigger'] = True if to_delete_ids else False

            # Persist a copy of source for callback indexing
            st.session_state['_prod_source_df'] = work_df.copy()
            st.data_editor(
                work_df,
                key='prod_editor',
                hide_index=True,
                column_config={'x': st.column_config.CheckboxColumn("🗑️", help="Tick to delete then enter password below"), 'ID': config['ID']},
                disabled=[c for c in work_df.columns if c not in ['x']],
                on_change=prod_delete_callback,
            )
            # Post-callback UI
            if st.session_state.get('_prod_delete_trigger'):
                ids = st.session_state.get('_prod_pending_delete_ids', [])
                if ids:
                    st.warning(f"Pending delete production IDs: {', '.join(map(str, ids))}")
                    st.session_state['_prod_pw_input'] = st.text_input("Password", type="password", key='prod_pw_input')
                    ...
                    with c2:
                        if st.button("Cancel", key='prod_cancel_delete'):
                            st.session_state['_prod_delete_trigger'] = False
                            st.session_state['_prod_pending_delete_ids'] = []
                            st.rerun()
                    with c3:
                        st.caption("Select/deselect checkboxes to modify selection.")
        else:
            st.markdown("#### Entries for Selected Date")
...
with tab3:
    ...
    try:
        ...
        if not issue_rows.empty:
            ...
            display_cols = ['spreader_roll_issue_id','issue_time','spell','bin_no','Jute Quality','wt_per_roll','no_of_rolls','Issued Weight (kg)','Issued Weight (MT)','breaker_inter_no']
            issue_display = issue_rows[display_cols].rename(columns={
                'spreader_roll_issue_id':'_issue_id',
                'issue_time':'Hour',
                'spell':'Spell',
                'bin_no':'Bin',
                'wt_per_roll':'Wt/Roll (kg)',
            ...
            PASSWORD = "9454368"
            issue_display['Delete'] = False
            issue_display['ID'] = issue_display['_issue_id']
            # Mark total row (last) as non-deletable
            if len(issue_display):
                issue_display.at[issue_display.index[-1], 'Delete'] = ''
                issue_display.at[issue_display.index[-1], 'ID'] = ''
            display_order_issue = ['Delete','ID','Hour','Spell','Bin','Jute Quality','Wt/Roll (kg)','Rolls','Issued Weight (kg)','Issued Weight (MT)','Breaker Inter']
            work_issue_df = issue_display[display_order_issue].copy()
            config_issue = {
                'Delete': st.column_config.CheckboxColumn("🗑️", help="Select issue rows then click Delete Selected", default=False),
                'ID': st.column_config.TextColumn("ID", help="spreader_roll_issue_id", disabled=True)
            }
            # Initialize session state for issue deletion callback
            if '_issue_pw_input' not in st.session_state:
                st.session_state['_issue_pw_input'] = ''
            if '_issue_delete_trigger' not in st.session_state:
                st.session_state['_issue_delete_trigger'] = False
            if '_issue_pending_delete_ids' not in st.session_state:
                st.session_state['_issue_pending_delete_ids'] = []

            # Align with pattern: rename Delete -> x for internal processing
            work_issue_df = work_issue_df.rename(columns={'Delete': 'x'})
            if len(work_issue_df) and work_issue_df.iloc[-1]['ID'] == '':
                work_issue_df.at[work_issue_df.index[-1], 'x'] = False
            try:
                work_issue_df['x'] = work_issue_df['x'].astype('boolean')
            except Exception:
                pass

            def issue_delete_callback():
                edited_rows = st.session_state['issue_editor']['edited_rows'] if 'issue_editor' in st.session_state else {}
                to_delete = []
                for ridx, edits in edited_rows.items():
                    try:
                        if edits.get('x') is True:
                            row_id_val = st.session_state['_issue_source_df'].iloc[ridx]['ID']
                            if row_id_val not in ('', None):
                                to_delete.append(row_id_val)
                    except Exception:
                        continue
                st.session_state['_issue_pending_delete_ids'] = to_delete
                st.session_state['_issue_delete_trigger'] = True if to_delete else False

            st.session_state['_issue_source_df'] = work_issue_df.copy()
            st.data_editor(
                work_issue_df,
                key='issue_editor',
                hide_index=True,
                column_config={'x': st.column_config.CheckboxColumn("🗑️", help="Tick to delete then enter password below"), 'ID': config_issue['ID']},
                disabled=[c for c in work_issue_df.columns if c not in ['x']],
                on_change=issue_delete_callback,
            )
            if st.session_state.get('_issue_delete_trigger'):
                ids = st.session_state.get('_issue_pending_delete_ids', [])
                if ids:
                    st.warning(f"Pending delete issue IDs: {', '.join(map(str, ids))}")
                    st.session_state['_issue_pw_input'] = st.text_input("Password", type="password", key='issue_pw_input')
                    from batching.spreader_roll_issue import delete_spreader_roll_issue
                    c1, c2, c3 = st.columns(3)
                    with c1:
                        if st.button("Confirm Delete", key='issue_confirm_delete'):
                            if st.session_state['_issue_pw_input'] == PASSWORD:
                                deleted_ct = 0
                                for iid in ids:
                                    if delete_spreader_roll_issue(iid):
                                        deleted_ct += 1
                                st.success(f"Deleted {deleted_ct} / {len(ids)} issue entries")
                                st.session_state['_issue_pw_input'] = ''
                                st.session_state['_issue_delete_trigger'] = False
                                st.session_state['_issue_pending_delete_ids'] = []
                                st.rerun()
                                ...
                                st.error("Incorrect password – no deletions performed.")
                    with c2:
                        if st.button("Cancel", key='issue_cancel_delete'):
                            st.session_state['_issue_delete_trigger'] = False
                            st.session_state['_issue_pending_delete_ids'] = []
                            st.rerun()
                    with c3:
