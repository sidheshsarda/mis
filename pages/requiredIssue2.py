...
# Containers for top-of-page tables
top_container = st.container()
calculated_targets_df = None
calculated_targets_caption = ""
jute_totals_df = None
jute_totals_caption = ""
target_stock_snapshot_df = None
target_stock_snapshot_caption = ""
required_issue_summary_df = None
...
if plan_df.empty:
    ...
    if plan_dates:
        ...
        calculated_targets_df = targets_df.copy()
        calculated_targets_caption = (
            f"Batch plan query window: {range_start.strftime('%Y-%m-%d')} to {range_end.strftime('%Y-%m-%d')}"
        )
        ...
        if not target_long.empty:
            ...
            if date_columns:
                ...
                st.markdown("###### Batch Plan Totals by Date")
                ...
                if composition_df is not None and not composition_df.empty:
                    ...
                    if not comp_subset.empty:
                        ...
                        st.markdown("###### Batch Plan with Jute")
                        ...
                        st.markdown("###### Batch Plan with Jute — Contributions (MT)")
                        ...
                        if maturity_df is not None and not maturity_df.empty:
                            ...
                            st.markdown("###### Target Roll Stock (MT)")
                            ...
                            if target_stock_snapshot_rows:
                                ...
                                target_stock_snapshot_df = snapshot_df
                                target_stock_snapshot_caption = (
                                ...
                                required_issue_summary_df = summary_df
    else:
        st.info("Batch plan has no dates available for the selected window.")


with top_container:
    st.subheader("Planner Snapshot Overview")
    if all(df is None for df in [calculated_targets_df, jute_totals_df, target_stock_snapshot_df, required_issue_summary_df]):
        st.info("Planner tables will appear after batch plan data is available for the selected window.")
    else:
        if calculated_targets_df is not None:
            st.markdown("##### Calculated Targets")
            st.dataframe(calculated_targets_df, use_container_width=True, hide_index=True)
            if calculated_targets_caption:
                st.caption(calculated_targets_caption)
        if jute_totals_df is not None:
            st.markdown("##### Jute Quality Totals (MT)")
            st.dataframe(jute_totals_df, hide_index=True, use_container_width=True)
            if jute_totals_caption:
                st.caption(jute_totals_caption)
        if required_issue_summary_df is not None:
            st.markdown("##### Required Issue Snapshot")
            st.dataframe(required_issue_summary_df, use_container_width=True, hide_index=True)
        if target_stock_snapshot_df is not None:
            st.markdown("##### Target Roll Stock Snapshot (MT)")
            st.dataframe(target_stock_snapshot_df, hide_index=True, use_container_width=True)
            if target_stock_snapshot_caption:
